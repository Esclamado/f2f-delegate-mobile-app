...
@Component({
  selector: 'app-notification-enhance',
  templateUrl: './notification-enhance.page.html',
  styleUrls: ['./notification-enhance.page.scss'],
})
export class NotificationEnhancePage implements OnInit {
  ...
  redirectPage(data){
    if(data.status == '2'){
      ...
      .subscribe(r => {
        ...
        this.env.storage.get('notification_count').then((count) => {
          ...
          //console.log('count to see', count);

          if(count > 0){

            let notif_count = count - 1;
            //console.log('see me', notif_count);

            this.events.publish('notification_count', notif_count, Date.now());
          }
        });
      });
    }
    ...
    if((data.data.data_type == 'set_meeting_request' || data.data.data_type == 'reschedule_meeting_request') && (data.meeting_schedule && data.meeting_schedule.status == '2')){
      ...
      // this.router.navigate([data.url]);
      let url = data.url;
      url = url.replace("note", "note-enhance");
          ...
          sched.schedules.forEach((timeslot) => {
            if(timeslot.data){
              if(timeslot.data.id == meeting_schedule.time_slot_id){
                ...
                this.router.navigate(['meeting-details-enhance'], navigationExtras);
              }
            }
          });
        }
  ...
  async showRequestListModal(data) {
    ...
    if(meeting_schedule){
      ...
      let timeslot = {
        ...
        'end_time_orig': ts.formated_end_time,
      };
