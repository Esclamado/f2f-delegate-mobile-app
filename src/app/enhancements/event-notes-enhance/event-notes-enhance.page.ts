...
@Component({
  selector: 'app-event-notes-enhance',
  templateUrl: './event-notes-enhance.page.html',
  styleUrls: ['./event-notes-enhance.page.scss'],
})
export class EventNotesEnhancePage implements OnInit {
  ...
  async showDelegatesModal(){
    ...
    this.note.env.storage.set('note_prev_page', '/event-notes-enhance/'+this.event_id);
  ...
  gotoNotes(data){
    this.note.env.storage.set('note_prev_page', '/event-notes-enhance/'+this.event_id);
    this.note.env.redirect('/note-enhance/'+this.event_id+'/'+data.id);
  }
